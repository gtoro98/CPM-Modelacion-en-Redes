...
def write_string_on_temp(description, duration,predecesor_field, window1, counter):
    string = read()
    ...
    print("Actividad "+str(counter)+":  Descripción: "+ description.get() +"  Duración: "+str(duration.get())+"  Predecesor: "+str(predecesor))
...
def activity(counter): 
    ...
    predecesor_field.place(x= 475, y= 195)

    add = Button(window1, text="Agregar Actividad", command= lambda: write_string_on_temp(description, duration,predecesor_field, window1, counter),  width =20, bg='#FF5733', fg='#ffffff', font=font.Font(size=22, weight='bold')).place(x=300, y=280)
